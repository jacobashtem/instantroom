    ...
    <main class="grid grid-cols-12 gap-4 container max-w-6xl mx-auto mt-20 min-h-screen">
        <article class="col-span-12 px-4 rounded bg-white mt-12 ">
            <template v-if="!isGenerationStarted">
                ...
                <UCarousel arrows indicators v-slot="{ item }" :items="themes"
                :prev-button="{
                    class: 'z-20'
                    }"
                    :next-button="{
                ...
                :ui="{ item: 'snap-start' }">
                    <div class="relative select-none  transition-all">
                        ...
                        <img :class="isSelected(item) ? 'brightness-50' : 'brightness-75'"
                            :src="`/themes/${item}.webp`" width="300" draggable="false">
                ...
                <div class="shadow-2xl rounded-2xl mt-12 py-8 px-4  flex-col items-left grid grid-cols-1  xs:grid-cols-12 gap-4">
                    ...
                    <div class="col-span-4">
                        ...
                        <UButton type="submit" variant="solid"
                        :disabled="!isChosenImgSrc?.length || !selectedThemes?.length"
                        class="bg-sunsetOrange-500 disabled:bg-sunsetOrange-200 rounded-full px-6 w-full py-6 hover:bg-sunsetOrange-700 h-10 flex justify-center text-2xl font-light self-end"
                        @click="isSpendTokensModal = true">Generuj</UButton>
                    </div>
