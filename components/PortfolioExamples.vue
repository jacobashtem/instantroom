    ...
    <section id="portfolio" class="portfolio-section pt-14 pb:24 lg:pt-24 lg:pb-32 px-4 w-full max-w-6xl">
      <div class="mx-auto w-full">
        ...
        <div class="text-center mb-12">
          ...
          <p class="text-xl lg:text-2xl mt-3 font-light">Przygotuj się na remont bez stresu – z Instant Room masz wszystko pod kontrolą, a do tego odrobinę rozrywki na deser!
          </p>
        </div>
        ...
        <div class="text-center mt-8">
          <UButton @click="toggleProjects" variant="solid" class="focus:shadow-outline focus:outline-none tracking-wide font-semibold bg-sunsetOrange-500 hover:bg-sunsetOrange-700 text-gray-100 py-4 rounded-lg transition-all duration-300 ease-in-out text-lg px-4">
...
// Funkcja generowania tytułu z nazwy pliku
const generateTitleFromFilename = (filename) => {
  // Uzyskaj tylko nazwę pliku bez ścieżki katalogu
  const fileNameWithoutPath = filename.split('/').pop();

  // Podziel nazwę na części przy użyciu myślnika jako separatora
  const parts = fileNameWithoutPath
    .replace(/\.\w+$/, '')  // Usuń rozszerzenie pliku
    .split('-');            // Podziel nazwę na części na podstawie myślników

  // Zwróć ostatnią część po kapitalizacji pierwszej litery
  return parts.slice(-1)[0].charAt(0).toUpperCase() + parts.slice(-1)[0].slice(1);
};


const initialVisibleProjects = ref(6); // Początkowo widocznych 6 projektów
const showAllProjects = ref(false); // Kontroluje, czy wszystkie projekty są widoczne

const visibleProjects = computed(() => {
  return showAllProjects.value ? filteredProjects.value : filteredProjects.value.slice(0, initialVisibleProjects.value);
});
...
const buttonLabel = computed(() => (showAllProjects.value ? 'Zwiń projekty' : 'Wczytaj więcej'));
const buttonIcon = computed(() => (!showAllProjects.value ? 'mdi:chevron-double-down' : 'mdi:chevron-double-up'));
</script>
